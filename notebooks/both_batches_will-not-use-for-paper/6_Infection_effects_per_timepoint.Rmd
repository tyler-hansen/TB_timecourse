...
Model: ~ 0 + Infection + Timepoint + Donor_ID + technical confounders[batch + flow_cell + perc_GC + perc_Aligned + perc_Dups]
...
design <- model.matrix(~ 0 + Infection + Timepoint + Donor_ID + batch + flow_cell + perc_GC + perc_Aligned + perc_Dups, data = sample_info)
```
...
Model: ~ 0 + Timepoint:Infection + Timepoint + Donor_ID + batch + flow_cell + perc_GC + perc_Dups + perc_Aligned
...
design_nested <- model.matrix( ~ 0 + Timepoint:Infection + Timepoint + Donor_ID + batch + flow_cell + perc_GC + perc_Dups + perc_Aligned, data = sample_info)
```
...
```{r}
permuted_fits <- fit_permutations(perms = permuted_si_perTime, formula = "~ 0 + Timepoint:Infection + Timepoint + Donor_ID + batch + flow_cell + perc_GC + perc_Dups + perc_Aligned", v = v, n = 10)
```
...
```{r}
hist(as.matrix(null_pvalues)[, 48:57])
hist(vfit_nested$p.value[, 48:57])
```
...
timepoint_list <- list("TimepointT1:InfectionMtb", "TimepointT3:InfectionMtb", "TimepointT5:InfectionMtb", "TimepointT8:InfectionMtb", "TimepointT12:InfectionMtb", 
                       "TimepointT18:InfectionMtb", "TimepointT24:InfectionMtb", "TimepointT30:InfectionMtb", "TimepointT36:InfectionMtb", "TimepointT48:InfectionMtb")
...
number_DEGs <- data.frame(Time = factor(x = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"), levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48")), 
                          number_DEGs = c(199, 1380, 1921, 2045, 2023, 2139, 1983, 1938, 1930, 1828))
...
standard_errors <- data.frame("T1" = `TimepointT1:InfectionMtb_results`$coef_se, "T3" = `TimepointT3:InfectionMtb_results`$coef_se, "T5" = `TimepointT5:InfectionMtb_results`$coef_se,
                              ...
                              "T48" = `TimepointT48:InfectionMtb_results`$coef_se,
                              row.names = `TimepointT1:InfectionMtb_results`$gene_id) %>% as.matrix()
```
...
mashed_betas_V_long$Timepoint <- factor(mashed_betas_V_long$Timepoint, levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"))

mashed_lfsr_V_long <- pivot_longer(mashed_lfsr_V, cols = everything(), names_to = "Timepoint", values_to = "Mashed_lfsr")
mashed_lfsr_V_long$Timepoint <- factor(mashed_lfsr_V_long$Timepoint, levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"))
...
```{r}
number_DEGs_mash <- data.frame(Time = factor(x = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"), levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48")), 
                          number_DEGs = c(157, 1294, 1800, 1937, 1890, 2008, 1889, 1886, 1888, 1726))
...
```
```{r}
tmp1 <- dplyr::filter(`TimepointT48:InfectionMtb_results`, empirical_qval < 0.01 & (coefficients > 1 | coefficients < -1)) %>% dplyr::select(gene_id)
tmp2 <- dplyr::filter(mashed_result_sig, Timepoint == "T48")[, "gene_id"] %>% as.data.frame() %>% distinct()
tmp3 <- inner_join(tmp1, tmp2, by = "gene_id")
nrow(tmp1)
nrow(tmp2)
nrow(tmp3)
...
timepoint_results_list <- list("T1" = `TimepointT1:InfectionMtb_results`, "T3" = `TimepointT3:InfectionMtb_results`, "T5" = `TimepointT5:InfectionMtb_results`, "T8" = `TimepointT8:InfectionMtb_results`, 
                               ...
                               "T30" = `TimepointT30:InfectionMtb_results`, "T36" = `TimepointT36:InfectionMtb_results`, "T48" = `TimepointT36:InfectionMtb_results`)
...
gsea_sig_df$Timepoint <- factor(gsea_sig_df$Timepoint, levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"))
```
...
clusters_plot <- ggplot(mashed_betas_degs_MC_clust_long, aes(x = timepoint_hr, y=mean_centered_Beta, color = cluster_id)) +
  ...
  scale_x_continuous(breaks = seq(0,48,5)) +
  labs(title = "Behavior of kmeans clusters over time",
...
genes <- dplyr::select(mashed_betas_degs_MC_clust, gene_id, Cluster_7) %>%
