...
2. Population-specific DEs at each timepoint using all expressed genes as input. 
  -Nested model: ~ 0 + Timepoint:African_admixture + Timepoint + technical confounders (including sex)
...
To do this, seperate each condition and then run look at ancestry effect per timepoint. 

__Option A__
I will first try including the confounders in the model, like this:
~ 0 + Timepoint:African_admixture + Timepoint + batch + flow_cell + perc_GC + perc_Aligned + perc_Dups + sex

__Option B__
I will next try regressing out the confounders and then applying corrected expression to the model, like this:
expr ~ 0 + batch + flow_cell + perc_GC + perc_Aligned + perc_Dups + sex + __residuals__
__residuals__ ~ 0 + Timepoint:African_admixture + Timepoint 
...
#read in SI
sample_info <- readRDS("data/metadata_with-covariates_final_samples_withBatch.rds")
...
### Option A: Include everything in the model

Set up design
```{r}
design_A_Mtb <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + batch + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_Mtb)
design_A_NI <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + batch + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_NI)
```

Subset counts by infection by intersecting with new SI
```{r}
cts_Mtb <- cts[, row.names(sample_info_Mtb)]
cts_NI <- cts[, row.names(sample_info_NI)]

#if match, then this will print true
all(rownames(design_A_Mtb) == colnames(cts_Mtb))
all(rownames(design_A_NI) == colnames(cts_NI))
```

Voom transform counts
```{r}
dge_Mtb <- DGEList(cts_Mtb)
dge_Mtb <- calcNormFactors(dge_Mtb)
v_Mtb <- voom(dge_Mtb, design_A_Mtb, plot = T)

dge_NI <- DGEList(cts_NI)
dge_NI <- calcNormFactors(dge_NI)
v_NI <- voom(dge_Mtb, design_A_NI, plot = T)
```

Fit and calc stats
```{r}
vfit_A_Mtb <- lmFit(v_Mtb, design_A_Mtb)
vfit_A_NI <- lmFit(v_NI, design_A_NI)

vfit_A_Mtb <- eBayes(vfit_A_Mtb)
vfit_A_NI <- eBayes(vfit_A_NI)
```
...
for (i in timepoint_list) {
  print(i)
  topTable(vfit_A_Mtb, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
  topTable(vfit_A_NI, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
}
```

```{r}
number_DEGs <- data.frame(Time = factor(x = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"), levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48")), 
                          Mtb = c(512, 1870, 1235, 63, 61, 22, 45, 92, 32, 25), 
                          NI = c(150, 1, 0, 0, 0, 0, 0, 0, 0, 303)) %>% pivot_longer(cols = 2:3, names_to = "Infection", values_to = "number_DEGs")

degs_plot <- ggplot(number_DEGs, aes(x = Time, y = number_DEGs, group = Infection, color = Infection)) +
  geom_line() +
  geom_text(aes(label = number_DEGs), nudge_y = 100) + 
  geom_point(size = 3) +
...
#### Look at batch 1/2 seperately
Subset sample info into NI and Mtb samples:
```{r}
sample_info_Mtb_batch1 <- dplyr::filter(sample_info_Mtb, batch == "batch1")
sample_info_Mtb_batch2 <- dplyr::filter(sample_info_Mtb, batch == "batch 2")

sample_info_NI_batch1 <- dplyr::filter(sample_info_NI, batch == "batch1")
sample_info_NI_batch2 <- dplyr::filter(sample_info_NI, batch == "batch 2")
```
...
Set up design
```{r}
design_A_Mtb_batch1 <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_Mtb_batch1)
design_A_Mtb_batch2 <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_Mtb_batch2)

design_A_NI_batch1 <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_NI_batch1)
design_A_NI_batch2 <- model.matrix(~ 0 + Timepoint:African_admixture + Timepoint + flow_cell + perc_GC + perc_Aligned + perc_Dups + Sex, data = sample_info_NI_batch2)
```

Subset counts by infection by intersecting with new SI
```{r}
cts_Mtb_batch1 <- cts[, row.names(sample_info_Mtb_batch1)]
cts_Mtb_batch2 <- cts[, row.names(sample_info_Mtb_batch2)]

cts_NI_batch1 <- cts[, row.names(sample_info_NI_batch1)]
cts_NI_batch2 <- cts[, row.names(sample_info_NI_batch2)]
```

```{r}
vfit_A_Mtb_batch1 <- DGEList(cts_Mtb_batch1) %>% calcNormFactors() %>% voom(design_A_Mtb_batch1) %>% lmFit(design_A_Mtb_batch1) %>% eBayes()
vfit_A_Mtb_batch2 <- DGEList(cts_Mtb_batch2) %>% calcNormFactors() %>% voom(design_A_Mtb_batch2) %>% lmFit(design_A_Mtb_batch2) %>% eBayes()

vfit_A_NI_batch1 <- DGEList(cts_NI_batch1) %>% calcNormFactors() %>% voom(design_A_NI_batch1) %>% lmFit(design_A_NI_batch1) %>% eBayes()
vfit_A_NI_batch2 <- DGEList(cts_NI_batch2) %>% calcNormFactors() %>% voom(design_A_NI_batch2) %>% lmFit(design_A_NI_batch2) %>% eBayes()
```
```{r}
#setup list to iterate through
timepoint_list <- list("TimepointT1:African_admixture", "TimepointT3:African_admixture", "TimepointT5:African_admixture", "TimepointT8:African_admixture", "TimepointT12:African_admixture", 
...
for (i in timepoint_list) {
  print(i)
  topTable(vfit_A_Mtb_batch1, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
  topTable(vfit_A_NI_batch1, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
}
```

```{r}
number_DEGs <- data.frame(Time = factor(x = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"), levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48")), 
                          Mtb = c(8, 59, 24, 13, 38, 0, 54, 3808, 20, 3603), 
                          NI = c(317, 11, 11, 5, 8, 5, 3, 6, 71, 55)) %>% pivot_longer(cols = 2:3, names_to = "Infection", values_to = "number_DEGs")

ggplot(number_DEGs, aes(x = Time, y = number_DEGs, group = Infection, color = Infection)) +
  geom_line() +
  geom_text(aes(label = number_DEGs), nudge_y = 100) + 
  geom_point(size = 3) +
  scale_color_manual(values = c("red", "grey60")) +
...
for (i in timepoint_list) {
  print(i)
  topTable(vfit_A_Mtb_batch2, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
  topTable(vfit_A_NI_batch2, number = Inf, p.value = 0.1, coef = i) %>% nrow() %>% print()
}
```

```{r}
number_DEGs <- data.frame(Time = factor(x = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48"), levels = c("T1", "T3", "T5", "T8", "T12", "T18", "T24", "T30", "T36", "T48")), 
                          Mtb = c(1674, 5073, 24, 13, 38, 0, 54, 3808, 20, 3603), 
                          NI = c(317, 11, 11, 5, 8, 5, 3, 6, 71, 55)) %>% pivot_longer(cols = 2:3, names_to = "Infection", values_to = "number_DEGs")
